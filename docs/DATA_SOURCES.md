...
| 8 | Cboe | 무키 | 풋콜비율 | 매시 |
| 9 | Google News RSS | 무키 | KR 키워드 뉴스 | 매시 |
...
## 9. Google News RSS — 무키

**현재 수집** (`news`)
- KR 키워드 3종(코스피·삼성전자·S-Oil) 최신 기사 → `news`

**추가로 가능한 것**
- 임의 키워드·기간·언어 검색 (로테이션 종목 동적 확장 가능)
- ⚠ 품질·중복 한계 → **네이버 검색 API로 교체 예정** (키 보유, 백로그)
...
| 소스 | 상태 | 용도 후보 |
|---|---|---|
| 네이버 검색 API | **키 보유, 미사용** | KR 뉴스 품질 업그레이드 + 종목별 뉴스 |
