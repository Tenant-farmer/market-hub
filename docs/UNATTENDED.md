...
- [ ] 워치독 경보 0건 (발생 시 원인 규명·해소하면 그 시점부터 재카운트)
  - 재카운트는 근거를 남겨야 인정: `collector_runs`에 collector='watchdog',
    status='resolved', message=**원인 문구**(필수)를 기록하면 그 시각 이후만 센다.
    판정 출력에는 전체 건수와 해소 사유를 **항상 같이** 찍는다(남용 방지)
- [ ] 수집 에러율 < 5% (외부 소스 일시 장애는 제외하되 기록)
- [ ] **전량 실패 수집기 0개** — 집계율만으로는 못 잡는다. 엔진 하트비트가 분모의
  58%라 하루 1회짜리 수집기 7개(backup 포함)가 전멸해도 1.9%로 합격이 나온다
...
### 2차 — 전략 성과 (에쿼티 **거래일** 20일 AND **2026-08-25 이후**)
- 원래 조건은 표본(20일)뿐이었다. 두 가지를 고쳤다(2026-07-28):
  - 표본을 **거래일만** 센다 — portfolio_snapshots는 매시간 돌아 주말에도 행이 생기고,
    장이 안 열린 날은 금요일 값 복사라 **수익률 0%인 가짜 관측**이 된다. 달력일로 세면
    20일이 8/11에 차서 조기 발송되고, 0% 날이 변동성을 낮춰 **α의 t값을 부풀린다**
  - 날짜 하한 **8/25** 추가(사용자 결정) — 거래일 20일은 8/20경에 겨우 차서 여유가 없다
